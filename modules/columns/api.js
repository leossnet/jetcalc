...
var HeadersLoader = (new function(){
	...
	self.Load = function(done){
		...
		Header.find().isactive().lean().sort({IndexHeader:1}).exec(function(err,Headers){
			var IndStr = {}, Map = {};
			...
			var roots = _.filter(Headers,{CodeParentHeader:""});
...
router.put('/colsetcol', HP.TaskAccess("IsColsetEditor"), function(req,res,next){
	...
	M.SetModel("colset",{CodeColset:req.body.CodeColset},function(err){
...
router.get('/headers', HP.TaskAccess("IsHeaderEditor"), function(req,res,next){
	...
	mongoose.model("header").find({CodeParentHeader:CodeHeader}).sort({IndexHeader:1}).isactive().exec(function(err,CP){
		return res.json(CP);
	})
})
...
router.put('/headers', HP.TaskAccess("IsHeaderEditor"), function(req,res,next){
	...
	M.Modify("header",{CodeParentHeader:req.body.CodeHeader},Headers,function(err){
			return res.json({});
	});
})
