...
var ReportPeriods = (new function() {
    ...
    self.Opened = function(done) {
        mongoose.model("periodedit").find({
            IsEdit: true
        }, "-_id CodePeriod CodeRole Year IsEdit").isactive().lean().exec(function(err, Set) {
            return done(err, Set);
        })
    }

    self.Roles = function(done) {
        mongoose.model("doc").find({
            IsInput: true
        }, "-_id CodeRole").isactive().lean().exec(function(err, Rs) {
            return done(err, _.uniq(_.map(Rs, "CodeRole")));
        })
    }

    self.ReportGroups = function(done) {
        mongoose.model("periodgrp").find({
            ForPermits: true
        }, "-_id CodePeriodGrp").isactive().lean().exec(function(err, Rs) {
    ...
    self.Periods = function(done) {
        self.ReportGroups(function(err, Arr) {
            ...
            mongoose.model("period").find({
                IsReportPeriod: true
            }, "-_id CodePeriod Link_periodgrpref").populate({
                ...
                PS.forEach(function(P) {
                    var PG = _.first(P.Link_periodgrpref),
                        Gr = "NONE";
...
var MapPeriods = (new function() {
    ...
    self.Load = function(done) {
        ...
        var Result = {},
            Fields = ["-_id", "CodeReportPeriod", "ReportYear", "IsOptional", "CodePeriod"];
        Links.find({}, Fields.join(" ")).lean().isactive().sort({
            IndexReportPeriod: 1
            ...
            if (!_.isEmpty(Periods)) {
                Periods.forEach(function(P) {
                    ...
                    self.Map[P.CodePeriod].push({
                        period: P.CodeReportPeriod,
                        year: P.ReportYear,
            ...
            Worker.get(function(err, Info) {
                ...
                ToGroup.forEach(function(CodePeriod) {
                    ...
                    rArr.forEach(function(G) {
                        if (Inf && Inf.Grps && Inf.Grps.indexOf(G) != -1) {
...
router.put('/update', HP.TaskAccess("IsPeriodEditTunner"), function(req, res, next) {
    ...
    PED.find({
        ...
        req.body.Value && req.body.Value.forEach(function(V) {
            NewArray.push(_.merge(V, {
                Year: Year
            }));
        })
        var ToSave = [];
        Current.forEach(function(C) {
            var F = _.find(NewArray, {
                Year: C.Year,
                CodePeriod: C.CodePeriod,
                CodeRole: C.CodeRole
            });
            if (!F && C.IsEdit) {
                C.IsEdit = false;
        ...
        NewArray.forEach(function(N) {
            var F = _.find(Current, {
                Year: N.Year,
                CodePeriod: N.CodePeriod,
                CodeRole: N.CodeRole
            })
            if (F && !F.IsEdit) {
                F.IsEdit = true;
