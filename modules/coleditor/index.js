var MColEditor = (new function() {
	...
	self.SaveChanges = function(){
		...
		var ToSave = {colsetcol:{},col:{}};
		var FieldTranslate = {InitialFormula:"Formula",InitialName:"NameColsetCol"}
		for (var Key in Changes){
			var K = Key.split("_"), F = _.last(K), Ind = parseInt(_.first(K)), T = _.includes(["Link_coltag","IsFormula","InitialFormula"],F) ? "col":"colsetcol", FV = _.isEmpty(FieldTranslate[F]) ? F:FieldTranslate[F];
			var Col = self.AllRows[Ind];
			if (_.isEmpty(ToSave[Col.CodeColsetCol])) ToSave[Col.CodeColsetCol] = {};
			ToSave[Col.CodeColsetCol][FV] = Col[F];
		}
		self.rPut("savechanges",{Context:CxCtrl.Context(),Changes:ToSave},function(){
			self.Show();
		})		
	}
    ...
    self.LoadCols = function(done){
    	self.rGet("cols",CxCtrl.Context(),function(Rows){
			...
			var ColCFG = {
				...
				"ShowName":["middle_text",true,400,{renderer:HandsonTableRenders.TreeRender}]
			}
        	switch(self.Mode()){
        		case "Filter":
        	...
        	for (var FieldName in ColCFG){
        		...
        		Columns.push(_.merge({type:inf[0],data:FieldName,title:Tr(FieldName),readOnly:inf[1]},_.isEmpty(inf[3])?{}:inf[3])); ColWidths.push(inf[2]);
        	}
