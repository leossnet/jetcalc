var CxCtrl = (new function(){
    ...
    self.IsShowMonthPlan = ko.observable(false,{  persist: 'cxShowMonthPlan' });
    self.PageName      = ko.observable(null,{  persist: 'cxPage' });
    ...
    self.CxPermDoc = function(){
        return {CodeDoc:self.CodeDoc(),CodeObj:self.CodeObj(),Year:self.Year(),CodePeriod:self.Override.CodePeriod() || self.CodePeriod()};
    }
    ...
    self.AfterShowUpdate = {document:-1,type:-1};
    ...
    self.UpdateViewShow = function(){
        ...
        self.Error(null);
        if (MPrint.IsPrint()) self.AfterShowUpdate = {document:-1,type:-1};
    ...
    self.Update = function(type,value){
        ...
        switch (type){
            ...
            case "CodeDiv":
            case "CodeOtrasl":
                var Remap = {CodeRegion:"region", CodeDiv:"div", CodeOtrasl:"otrasl", CodeGrp:"grp"};
        ...
        if (Updater==-1) return;
        var DocPlugin = _.find(ModuleManager.DocTabs(),{id:self.PageName()});
        if (DocPlugin){
    ...
    self.Context = function(){
        ...
        if (self.IsAgregate()){
            try{
                Result.Agregate = _.find(MFavorites.Agregates()[self.AgregateType()],{code: self.Agregate()}).objs;
                Result.AgregateType = {Model:self.AgregateType(),Code:self.Agregate()};
    ...
    self.CodeDoc     = ko.observable(null,{  persist: 'cxCodeDoc' });
    self.CodeReport  = ko.observable('default');
    self.CodeObj     = ko.observable(null,{  persist: 'cxCodeObj' });
    self.ChildObj    = ko.observable(null,{  persist: 'cxChildObj' });
    self.CodeValuta  = ko.observable(null,{  persist: 'cxCodeValuta' });
    self.CodePeriod  = ko.observable(null,{ persist: 'cxCodePeriod' });
    self.Year        = ko.observable(null,{  persist: 'cxYear' });
    self.IsInput     = ko.observable(null,{  persist: 'cxIsInput' });
    self.CodeGrp     = ko.observable(null,{  persist: 'cxCodeGrp' });
    self.KvartPeriod = ko.observable(null,{  persist: 'cxKvartPeriod' })
    self.MesPeriod   = ko.observable(null,{  persist: 'cxMesPeriod' })
    self.GroupType   = ko.observable(null,{  persist: 'cxGroupType'});
    


    self.Override = {
        CodePeriod: ko.observable(null,{  persist: 'cxOvCodePeriod' }),
        Year: ko.observable(null,{  persist: 'cxOvYear' }),
        SetDivObj: ko.observable(false,{  persist: 'cxDivObj' }),
    ...
    self.defaultValuta = function(){
        var Valuta = "RUB";
        try{
            Valuta = _.find(MFavorites.Agregates().obj,{code: self.CodeObj()}).Valuta;
        } catch (e){
            ;
        }
        return Valuta;
    ...
    self.ChangeInitDocPath = function(){
        if (_.includes(window.location.href,'docview')) {
            var avParams = ['CodeObj','CodePeriod','Year',"CodeReport"];
