var ModelRestrict = (new function(){
	...
	self.Restrictions = {
		docfolder:{
			CodeParentDocFolder:{CodeParentDocFolder:""}
		},
		docfolderdoc:{
			CodeDocFolder:{CodeParentDocFolder:{$ne:""}}
		},
		docrow:{
			CodeRow:{CodeParentRow:""}
		},
		docheader:{
			CodeHeader:{CodeParentHeader:""}
		}
	}
...
var ModelChooser = (new function(){
    ...
    self.UpdateSearch =  ko.computed(function() {
        var model = self.ModelName()+'',  query = self.SearchStr()+'';
        self.SearchResults([]);
...
var Catalogue = (new function(){
	...
	self.SearchModel = function(model, query, callback) {
		self.DoSearch({model:model,q:query},callback);
    }	

    self.SearchModelExtended = function(model, query, callback) {
		self.DoSearch({model:model,q:query,fields:ModelChooser.Fields[model],extend:true},callback)
    }

    self.SearchModelUnlimit = function(model, query, callback) {
		self.DoSearch({model:model,q:query,unlimit:true},callback)
    }
	...
	self.UpdateDom = function(){
		$('[data-catalogue]').each(function(index,node){
			try{
				...
				var tr = nod.split("_"), M = tr.shift(), C = tr.join("_");
	...
	self.GetAll = function(model){
		if (model=='year'){
			var R = [], C = Number(moment().format("YYYY"));
			for (var i = C-5; i<C+2; i++){
				var str = i+'';
				R.push({id:str,name:str});
			}
			return R;
		}
		return [];
...
var ModelTableEdit = (new function(){
	...
	self.Search = ko.observable("").extend({ throttle: 600 });
	...
	self.Validate = function(done){
		...
		var L = self.LoadedModel(), CodeField = L.Code, CodeValue = L[CodeField](), _id = L._id ? L._id(): null;
...
var ModelEdit = (new function(){
			...
			if (Field.indexOf("Link_")==0 && Doc[Field].length){
				...
				if (typeof Test =='object'){ // populated
					...
					Doc[Field].forEach(function(AL){
						var N = MModels.Create(ModelName,AL,{IsEdit:false});
						NewArr.push(N);
					})
	...
	self.AddLink   = function (LinkName){
		...
		console.log(Init);
		var A = MModels.Create(LinkName,Init,{IsEdit:true});
