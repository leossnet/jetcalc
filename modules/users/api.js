...
router.post('/requestaccept', LIB.Require(['CodeObj']), HP.TaskAccess("IsRequestApprover"), function (req, res, next) {
    var D = req.body,
        Link = mongoose.model('userrequest'),
        User = mongoose.model("user");
    mongoose.model("request").findOne({
        CodeRequest: D.CodeRequest
    }).isactive().exec(function (err, Request) {
        ...
        Request.save(req.user.CodeUser, function (err) { // 1. Update Request
            mongoose.model('obj').findOne({
                CodeObj: Request.CodeObj
            }).isactive().lean().exec(function (err, Obj) {
                ...
                mongoose.model("taskprivelege").find({
                    ...
                    mongoose.model("usertask").find({
                        ...
                        mongoose.model("user").find({
                            ...
                            var obj4push = {};
                            obj4push[O.NameObj] = _.filter(Real, function (R) {
                                return R.Mail && R.Mail.length;
                            });
                            ToSend.push(obj4push);
                            works -= 1;
                        })
                    })
                })
...
router.get('/sendrequizites', LIB.Require(['CodeUser']), HP.TaskAccess("IsUserAcceptor"), function (req, res, next) {
    var Mailer = require(__base + 'src/mailer.js'),
        CodeUser = req.query.CodeUser;
    mongoose.model("user").findOne({
        CodeUser: CodeUser
    }).isactive().exec(function (err, User) {
...
router.post('/useraccept', HP.TaskAccess("IsUserAcceptor"), function (req, res, next) {
    var D = req.body.User2Approve,
        User = mongoose.model("user");
    User.findOne({
        CodeUser: D.CodeUser
    }).isactive().exec(function (err, User2Approve) {
...
router.post('/userpermit', HP.TaskAccess("IsDocPermissionAssigner"), function (req, res, next) {
    var Data = req.body.userpermit,
        M = mongoose.model("userpermit"),
        CFG = M.cfg();
...
router.post('/usertask', HP.TaskAccess("IsFunctionAssigner"), function (req, res, next) {
    var Data = req.body.usertask,
        M = mongoose.model("usertask"),
        CFG = M.cfg();
...
router.delete('/usertask', HP.TaskAccess("IsFunctionAssigner"), function (req, res, next) {
    ...
    mongoose.model("usertask").findOne({
            CodeUserTask: CodeUserTask
        })
        .isactive()
...
router.get('/permits', HP.TaskAccess("IsDocPermissionAssigner"), function (req, res, next) {
    ...
    var RoleModel = mongoose.model("role"),
        PGroupModel = mongoose.model("periodgrp");
    RoleModel.find({}, "CodeRole NameRole IsExtended").isactive().lean().sort({
        NameRole: 1
    }).exec(function (err, Roles) {
        ...
        PGroupModel.find({
            ForPermits: true
        }, "CodePeriodGrp NamePeriodGrp").isactive().lean().exec(function (err, Grs) {
...
router.get('/permit',  HP.TaskAccess("IsDocPermissionAssigner"), function (req, res, next) {
    ...
    }, "CodePermitRole CodePermit CodePeriodGrp CodeRole DoBlock DoWrite DoRead").isactive().lean().exec(function (err, cRoles) {
        if (err) return next(err);
...
router.get('/user/:code',   HP.TaskAccess("IsUserAcceptor"), function (req, res, next) {
    ...
    mongoose.model("user").findOne({
            CodeUser: CU
        }, "CodeUser LoginUser NameUser JobTitle Mail WorkPhone MobilePhone UserPhoto CodeObj Link_usertask Link_userpermit")
        ...
        .lean().isactive().exec(function (err, User) {
            ...
            async.parallel({
                Permits: function (callback) {
                    ...
                    mongoose.model("permit").find({
                            CodePermit: {
                                $in: CodePermits
                            }
                        }, "CodePermit NamePermit SNamePermit IsPublic IsPersonal Link_permitrole Link_userpermit")
                ...
                Request: function (callback) {
                    ...
                    mongoose.model("request").findOne({
                        CodeRequest: _.first(CodeRequests)
                    }).isactive().lean().exec(callback);
                }
...
router.get('/avatar/:CodeUser', function (req, res, next) {
    ...
    mongoose.model('user').findOne({
        CodeUser: CodeUser
    }, 'NameUser UserPhoto').isactive().lean().exec(function (err, U) {
        if (!U) return next("User not found");
        if (_.isEmpty(U.UserPhoto)) {
            var f = config.staticDir + '/media/fempty.png',
                m = config.staticDir + '/media/mempty.png',
                result = m;
            try {
