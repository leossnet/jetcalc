...
function checkStatus(done) {
	...
	exec("git fetch https://JC_USER:derparole12j@bitbucket.org/zubra12/ugmk.git",function(error, stdout, stderr) {
		if (!error) {
			console.log('here');
			var exec = require('child_process').exec;
...
function pull(done) {
	// checkStatus(function(err,status) {
		...
		if (true || status == "Need to pull" || status == "Diverged") {
			var exec = require('child_process').exec;
			var cmd = "git pull https://JC_USER:derparole12j@bitbucket.org/zubra12/ugmk.git";
