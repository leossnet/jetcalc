...
module.exports = function(schema, options) {
    ...
    schema.methods.userSaveAsync = async function (CodeUser) {
        console.log('userSaveAsync - start', CodeUser)
        const self = this
        self.UserEdit = CodeUser
        return new Promise((resolve, reject) => {
            console.log('userSaveAsync - save start', CodeUser, self)
            self.save(error => {
                console.log('userSaveAsync - save error', error)
                if (error) {
                    return reject(error)
                } else {
                    console.log('userSaveAsync - save done', error)
    ...
    schema.methods.userSave = function (CodeUser, done) {
    ...
    schema.methods.SetCode = function(done) {
        ...
        mongoose.model(Config.ModelName).count(Check).isactive().exec(function(err, СС) {
            ...
            var CheckCh = {};
            CheckCh[CodeField] = {
                $regex: new RegExp("^" + CodeValue + "_[0-9]+$")
            };
    ...
    schema.pre('remove', function(next, done) {
        ...
        var Update = {};
        Update[CodeField] = mongoose.Types.ObjectId() + '_' + self[CodeField];
    ...
    var BusInform = function() {
        var self = this,
            CFG = self.cfg(),
            ModelName = CFG.ModelName;
        Bus.Emit("modelchange", ModelName);
    }

    schema.post('remove', BusInform);
	schema.post('save', BusInform);
}
