...
var SignupHelper = (new function(){
    ...
    self.UserAcceptors = function(CodeRequest,done){
        // Пользователь
        mongoose.model("userrequest").findOne({CodeRequest:CodeRequest},"-_id CodeUser").isactive().lean().exec(function(err,U){
            if (!U) return done("no_userrequest");
            mongoose.model("user").findOne({CodeUser:U.CodeUser},"-_id CodeObj").isactive().lean().exec(function(err,O){
                if (!O || !O.CodeObj) return done("nouser");
                mongoose.model("objgrp").find({CodeObj:O.CodeObj},"CodeObjGrp").isactive().lean().exec(function(err,Ls){
                    if (!O) return done("nouser");
                    mongoose.model("taskprivelege").find({CodePrivelege:"IsUserAcceptor"},"-_id CodeTask").isactive().lean().exec(function(err,Tasks){
                        var Tasks = _.map(Tasks,"CodeTask");
                        if (!Tasks.length) return done(null,[]);
                        mongoose.model("usertask").find({CodeTask:{$in:Tasks}},"-_id CodeUser CodeObj CodeObjGrp").isactive().lean().exec(function(err,Users){
                            ...
                            mongoose.model("user").find({CodeUser:{$in:Announce}},"-_id NameUser Mail").isactive().lean().exec(function(err,Real){
                                var ToSend = _.filter(Real,function(R){
                                    return R.Mail && R.Mail.length;
                                })
                                return done(null,ToSend);                            
    ...
    self.ConfirmRequest = function(CodeRequest,done){
        ...
        mongoose.model("request").findOne({CodeRequest:CodeRequest}).isactive().lean().exec(function(err,R){
