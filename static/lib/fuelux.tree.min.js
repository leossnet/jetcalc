    ...
    Tree.prototype = {
        ...
        populate: function populate($el) {
            ...
            this.options.dataSource(treeData ? treeData : {}, function (items) {
                ...
                var stree = localStorage.fuelux_tree;
                if (!stree) {
                    stree = '{}'
                };
                ...
                $.each(items.data, function (index, value) {
                    ...
                    if (value.type === 'folder') {
                        ...
                        if (stree[value.model] && stree[value.model].indexOf(value.code) != -1) {
                            self.openFolder($entity);
                        }
        ...
        openFolder: function openFolder(el, ignoreRedundantOpens) {
            ...
            var $parent = ($treeFolderContent.hasClass('tree')) ? $treeFolderContent : $treeFolderContent.parent();
            var treeData = $parent.data();
            var stree = localStorage.fuelux_tree;
            if (!stree) {
                stree = '{}'
            ...
            stree = JSON.parse(stree);
            if (stree[treeData.model]) {
                if (stree[treeData.model].indexOf(treeData.code) === -1) {
                    stree[treeData.model].push(treeData.code);
                }
            } else {
                stree[treeData.model] = [treeData.code];
            }
            localStorage.setItem('fuelux_tree', JSON.stringify(stree));
        ...
        closeFolder: function closeFolder(el) {
            ...
            stree = JSON.parse(stree);
            if (stree[treeData.model] && stree[treeData.model].indexOf(treeData.code) != -1) {
                stree[treeData.model].remove(treeData.code);
            }
            localStorage.setItem('fuelux_tree', JSON.stringify(stree));
            var $treeFolderContentFirstChild = $treeFolderContent.eq(0);
