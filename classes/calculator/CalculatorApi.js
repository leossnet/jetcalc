...
router.post('/api/cell/validateformula', function(req, res, next) {
    ...
    DivHelper.get(function(err, DivInfo) {
        ...
        var Frm = req.body.Formula + "",
            Vars = Frm.match(R.Var),
            ToTest = {
                Row: [],
                Col: [],
                Period: [],
...
router.get('/api/form/info', function(req, res, next) {
    ...
    DocWorker.get(function(err, Doc) {
        if (!Doc) {
            return res.json({
                err: "Документ " + Context.CodeDoc + " не найден"
            });
        }
    ...
    self.CalculateWithAF = function(done) {
        ...
        async.parallel({
            ...
            afcells: function(done) {
                RabbitManager.CalculateDocument(_.merge(self.Context, {
                    IsAFOnly: true
                }), function(err, Result) {
            ...
            for (var CodeObj in ResultAll) {
                ...
                for (var CellName in ResultAll[CodeObj].Cells) {
                    ...
                    if (!AddCells[NewName]) {
                        AddCells[NewName] = {
                            Type: 'FRM',
                            FRM: (ResultAll[CodeObj].Cells[CellName].FRM + '').replace('#' + CodeObj + '?', '#' + Key2Set + '?'),
                            Value: 0
                        };
                    }
        ...
        RabbitManager.CalculateDocument(self.Context, function(err, Result) {
            ...
            Objs.get(function(err, Objs) {
                ...
                Objs.Objs.forEach(function(O) {
                    IndexedObjs[O.CodeObj] = _.merge(O, {
                        CodeGrp: Objs.Grp.CodeGrp
                    });
                })
                var ObjSums = {};
...
var CellSaveHelper = (new function() {
    ...
    self.EnsurePresence = function(CodeUser, Cells, done) {
        ...
        async.each(ToSync, function(info, cb) {
            ...
            var Q = {};
            if (info.model == 'user') Q["LoginUser"] = {
                $in: Codes
            };
            else {
                Q[info.field] = {
            ...
            Tasks[ModelName] = function(ModelName, Field) {
                return function(done) {
                    mongoose.model(ModelName).find({
                        IsFormula: true,
                        IsActive: true
                    }, Field + " OldFormula").lean().exec(function(err, Objs) {
                        var Result = {};
                        Objs.forEach(function(O) {
                            Result[ModelName + "." + O[Field]] = O.OldFormula;
                        });
                        return done(err, Result);
                    })
                }
...
var FormulaUpdateSyntacis = function() {
    ...
    self.LoadInfo = function(done) {
        mongoose.model("prod").find({
            IsActive: true
        }, "-_id CodeProd").lean().exec(function(err, Prods) {
...
router.post('/api/cell/history', function(req, res, next) {
    ...
    mongoose.model("row").findOne({
        CodeRow: CodeRow
    }, "CodeRow NameRow").isactive().lean().exec(function(err, Row) {
...
router.get('/api/row/finddocument', function(req, res, next) {
    var DocRowH = new Form.DocRow({
        UseCache: true
    });
    DocRowH.get(function(err, CacheRaw) {
        var Cache = _.merge(CacheRaw.WithChildObjs, CacheRaw.NoChildObjs);
        mongoose.model('row').findOne({
            CodeRow: req.query.Row
        }, 'CodeRow NameRow rowpath').lean().exec(function(err, R) {
            if (err) return next(err);
